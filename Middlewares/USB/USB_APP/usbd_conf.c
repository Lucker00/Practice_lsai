...
void HAL_PCD_MspInit(PCD_HandleTypeDef *hpcd)
{
    ...
    if (hpcd->Instance == USB_OTG_FS)
    {
        ...
        HAL_NVIC_SetPriority(OTG_FS_IRQn,5, 0);             /* 抢占优先级设置为0,响应优先级为3 */
...
void HAL_PCD_ResetCallback(PCD_HandleTypeDef *hpcd)
{
    ...
    {
        case PCD_SPEED_HIGH:
            CLOGI("USB Device Library  [HS]");
            speed = USBD_SPEED_HIGH;
            break;

        case PCD_SPEED_FULL:
            CLOGI("USB Device Library  [FS]");
